...
void BlurApp::Draw()
{
	...
	mBlurFilter->Execute(mCommandList.Get(), mPostProcessRootSignature.Get(), 
		mPSOs["horzBlur"].Get(), mPSOs["vertBlur"].Get(), CurrentBackBuffer(), 4, mBlurSigma);
...
void BlurApp::OnKeyboardInput()
{
    if(GetAsyncKeyState('W') & 0x8000) {
		mBlurSigmaUpdateSpeed = 1.0f;
       // std::cout << mBlurSigmaUpdateSpeed << std::endl;
    }
      ...
      //  std::cout << mBlurSigmaUpdateSpeed << std::endl;
        ...
        mBlurSigmaUpdateSpeed = 0.0f;
    }

    mBlurSigma += mBlurSigmaUpdateSpeed * mTimer.DeltaTime();

    if (mBlurSigma < 0.1f) {
        mBlurSigma = 0.1f;
    }
    else if (mBlurSigma > 5.0f) {
        mBlurSigma = 5.0f;   
    }

    // std::cout << mBlurSigma << std::endl;
}
 
void BlurApp::UpdateCamera()
{
...
void BlurApp::UpdateMaterialCBs()
{
